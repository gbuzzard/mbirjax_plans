"""mg35 -- THE 2048-CLASS CONFIRMATION OF THE SORTED FORWARD.

WHY THIS RUN EXISTS.  mg34 passed the composed gate's first half: the full
GPU suite on the staged tree, then the sorted-against-per-tap A/B at the
512- and 1024-class parallel cells, 1.03x to 1.89x with every value gate
holding.  This run is the second half, the mg24 pattern: the same A/B at
the 2048-class parallel cell, where the values block is 24 GB and the
sorted route's per-view row gather has never been measured.  The counts
are three and four devices, because the capacity table refuses one and
two at this class; with no one-device arm, the value gate is the
mode-pair comparison at each count, at the standing 1e-3.

THE PROTOCOL is mg34's (mg1's recon protocol, fresh subprocess per arm,
count pinned, MBIRTORCH_SORTED_FORWARD per arm), with the staging at
FOUR devices: a 2048-class forward projection does not fit one.

Run:
    <torch python> mg35_sorted_2k.py            on a 4-GPU node
    MG35_DRY=1 <python> mg35_sorted_2k.py       print the plan and stop
    MG35_SMOKE=1 <python> mg35_sorted_2k.py     tiny CPU plumbing pass

Configuration is by environment variable only; there is no command line.
    MG35_RESULTS=<dir>      the jsonl and the staged 33 GB sinogram
    MG35_SMOKE=1 / MG35_DRY=1
    MG35_REPEATS=3          warm repeats after the discarded cold pass
    MG35_VARIANTS=a,b       a subset, by variant id
"""
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
SMOKE = os.environ.get("MG35_SMOKE", "0") == "1"
DRY = os.environ.get("MG35_DRY", "0") == "1"
...
WARM_REPEATS = max(1, int(os.environ.get("MG35_REPEATS", "3")))
...
def _sino_path(geometry, cell):
    ...
    return os.path.join(RESULTS_DIR, f"mg35_sino_{geometry}_{cell[0]}.npy")
...
def build_model(geometry, cell, cpu_devices=None):
    ...
    if geometry == "parallel":
        ...
        raise ValueError(f"mg35 has no model construction for geometry "
                         f"{geometry!r}")
...
def run_stage(cfg):
    ...
    # The 2048-class staging takes an EXPLICIT four-device layout.  The
    # first submission left the stage on the automatic branch under the
    # count pin, the settle landed on ONE device, and the projection ran
    # out of memory assembling the 30.5 GiB sinogram beside the 29.6 GiB
    # phantom and the values cylinders -- the explicit branch shards all
    # three.  That a pinned automatic settle chose one device for a
    # projection this size, instead of widening or refusing, is recorded
    # as an observation for the device-policy ledger, not chased here.
    explicit = ([f"cuda:{i}" for i in range(4)] if not SMOKE
                else pin_devices_for(1))
    model = build_model(geometry, cell, cpu_devices=explicit)
...
def spawn(cfg):
    ...
    """
    os.makedirs(RESULTS_DIR, exist_ok=True)
    cfg_path = os.path.join(RESULTS_DIR, f"_mg35_cfg_{cfg['job_id']}.json")
    out_path = os.path.join(RESULTS_DIR, f"_mg35_out_{cfg['job_id']}.json")
...
def build_plan():
    ...
    keep = _strict_subset("MG35_VARIANTS", all_variant_ids())
    plan = []
    for cell in cells():
        for geometry in GEOMETRIES:
            ...
            plan.append(dict(kind="stage", geometry=geometry, cell=list(cell),
                             n_dev=(4 if not SMOKE else 1), mode="srt",
    ...
    if not plan:
        raise ValueError("MG35_VARIANTS selects no variant")
    return plan
...
def print_plan(plan):
    ...
    print(f"mg35 the sorted-forward composed A/B: {len(variants)} variant(s) "
...
def main():
    ...
    out_path = os.path.join(RESULTS_DIR,
                            f"mg35_sorted_2k_{RUN_LABEL}_{stamp}.jsonl")
    print_plan(plan)
    print(f"\nrunning -> {out_path}", flush=True)
    started = time.time()
    rows = []
    with open(out_path, "w") as sink:
        header = dict(row="run_header", script="mg35_sorted_2k.py",
...
def summarize(rows, plan, out_path):
    ...
    """
    print(f"\n===== mg35 the sorted-forward composed A/B ({out_path}) =====")
    broken, findings = [], []
    by_variant, stages = {}, []
