"""mg12 -- does giving the column gather's cross-device copies a stream of
their own close the transfer stall the forward projection is sitting in?

WHAT IS BEING MEASURED, in one paragraph.  On more than one GPU the forward
projection gathers batches of voxel columns from the GPUs that own the slices
onto the GPU that owns the views, and then projects them.  That shape -- the
column gather -- is the library default on the tree this job starts from.  mg11
measured it and left one thing unexplained: on four H100s at the 1024 cell the
forward funnel's per-device wall was 16.3 s while the projection kernels
themselves were only busy for 12.4 s of it (pixel batch 4096; at batch 32768 the
same pair reads 10.0 and 9.3).  The ~4 s difference is the device sitting idle
while values move to it.  This job measures whether two candidate changes close
that difference.

THE THREE TREES, one per arm, never two in one process.
    control    the committed tree at 3e9c183c192b8c6f320a2cacabcf5ba43fea6fbf.
               Each batch is gathered, then projected, then the next batch is
               gathered.  Nothing overlaps by construction.
    prefetch   the same tree with prefetch_only.patch applied.  The driver
               issues batch k+1's gather BEFORE projecting batch k, but every
               copy still rides the device's default CUDA stream -- the same
               stream the projection runs on.
    streams    the same tree with five files replaced.  On top of the prefetch
               ordering, each device gets a CUDA stream used for nothing but
               these copies, and each batch carries its own event so a
               projection waits for its own batch's copies and not for the
               batch being gathered ahead of it.

WHAT EACH ARM IS FOR, stated before the numbers so a result cannot be fitted to
a story afterwards.
    THE PREFETCH ARM IS A PREDICTION CHECK, and the prediction is that it
changes NOTHING.  A CUDA stream runs the work it was given one item at a time
and in order.  If the copies are on the same stream as the projections, then
issuing a copy earlier only moves its place in a queue that still has to run
everything in turn -- the device cannot do the copy and the projection at once
whatever order they were handed over in.  So the prefetch arm's bracket should
land on the control's, inside the pass-to-pass spread.  If it does not, the
reasoning above is wrong somewhere and the streams result has to be re-read in
that light.  This arm exists precisely so that the streams arm's result cannot
be credited to the prefetch ordering it also contains.
    THE STREAMS ARM IS THE ONE EXPECTED TO CLOSE THE STALL.  Copies on their
own stream can run at the same time as a projection on the compute stream, and
the per-batch event is what keeps a projection from waiting on the batch that is
deliberately still moving.  If the reasoning holds, this arm's bracket falls
toward its busy time and its busy time does not move.

THE ONE GATE THAT CAN FAIL THIS INCREMENT is memory, and it is the streams arm
that is at risk.  Both overlays already charge one more gathered cylinder in the
memory ledger (COLUMN_GATHER_RESIDENTS goes from 2 to 3), because a device that
gathers ahead really does hold the next batch while it projects this one.  The
streams arm adds two further ways the MEASURED peak could outrun that charge:
torch's caching allocator keeps its blocks per stream, so a copy stream is a
second pool on the same device; and ``record_stream`` defers the reuse of a
block until the reading stream is done with it, which holds blocks live longer
than a plain free would.  mg11 measured this ledger's margin at 0.4 to 1.0
percent, so there is very little room.  modeled/measured at or above 1.00 passes;
below 1.00 is printed as FLOOR VIOLATION.

WHAT THIS FILE DOES NOT DO.  It draws no conclusion beyond two mechanical lines
per comparison, both defined below.  It does not decide whether a change ships.
It does not compare against any number measured in another job: control,
prefetch and streams are all measured HERE, on this node, in this process
ordering, against the same sinogram.

THE TWO VERDICT LINES, and the rule each is drawn by.  A "spread" is one arm's
own max-minus-min over its two warm passes, floored at SPREAD_FLOOR_FRAC of the
control arm's reading -- two passes is a weak estimator of run-to-run noise, so
the comparison refuses to resolve a difference smaller than that floor.
    STREAMS CLOSE THE STALL   when control_bracket - streams_bracket exceeds
                              the two arms' spreads added together.
    NO CHANGE                 otherwise.
    PREFETCH-ONLY PREDICTION: CONFIRMED when |prefetch_bracket -
                              control_bracket| is within the two spreads added
                              together -- the prediction was that nothing moves.
                              REFUTED otherwise.
Two further lines are printed only when they fire, and neither overrules a
verdict -- each says something about the reading a verdict rests on.
    BUSY CHANGED     when an overlay's busy time differs from the control's by
                     more than the two busy spreads together.  The overlays are
                     not supposed to touch what the kernels compute or how they
                     are launched, so a busy change means something other than
                     the transfer schedule moved, and every bracket reading in
                     that block has to be read with that in mind.
    UNSTABLE ARM(S)  when an arm's own two warm passes disagree by more than
                     ...
                     confirm whatever was predicted, so this is printed
                     immediately above the verdict lines of its block and again
                     beside them in the recap.

THE ARMS.  Five configurations, each run on all three trees, 15 measured arms,
plus two untimed sinogram generators.
    parallel  n=4  batch  4096     the cell the stall was measured at
    parallel  n=4  batch  8192
    parallel  n=2  batch  8192
    cone      n=4  batch  8192
    cone      n=2  batch  8192
All at mg9's, mg10's and mg11's cell (1024, 1008, 992), which gives 1008 slices
-- divisible by 1, 2 and 4, so no arm's arithmetic carries a padding term.  ARM
ORDER is configuration-major: the three trees of one configuration run
back-to-back.  That costs nothing and buys two things.  A truncated job yields
COMPLETE comparisons for the configurations it finished rather than a control
column and no treatment.  And if the node drifts over the hours the job runs,
the drift lands inside a comparison rather than between the control column and
the treatment column.

THE TWO KNOBS, and the ONE that is deliberately not set.
    model.forward_project_pixel_batch = B   the documented override of
        tomography_model.FORWARD_PIXEL_BATCH.  Every arm sets it.
    model.configure_devices(num_devices=n)  the documented device pin.
    MBIRTORCH_FORWARD_COLUMN_GATHER IS LEFT UNSET, on every arm, on purpose.
        The column gather is the DEFAULT on all three trees, so an unset switch
        is the shipped configuration and is what these numbers should be about.
        The runner removes the variable from every child environment rather than
        trusting it to be absent, and every arm asserts both that it is unset
        and that the library's own resolver, model._column_gather_forward(),
        answers True.  A stale export of '0' in a submitting shell would
        otherwise turn all fifteen arms into the banded walk without a word.

WHY THE DEVICE COUNT IS PINNED WITH configure_devices.  Calling it takes the
layout out of the library's hands: the count named is the count used, the
automatic device-count search never runs, and the widening SPEED FLOORS that
order that search never run either.  That is why it does not matter whether the
three trees carry the same floors table -- no arm consults one.  (In this build
they do carry the same table: the overlays replace five files and
_widening_floors.py is not among them.)  A second benefit is that there is no
device-count settle during the first reconstruction, so the projector object the
probes attach to is the one the timed passes use.
    THE COST, stated plainly: an explicitly configured layout is the caller's,
so the library does not build a memory ledger of its own for it, and
model.last_memory_ledger stays None.  The modeled peak below therefore comes
from this file's own call to the library's same entry point,
TomographyModel._build_memory_ledger.  That is not a weaker number here: mg11
pinned the other way, built both, and its rows show the two IDENTICAL on every
arm at this cell.  The row records which one it read, and reads
last_memory_ledger whenever the library did build one.

WHAT EVERY ARM PROVES ABOUT ITSELF.  A wrong PYTHONPATH would run three arms of
the same tree and quietly report "no change" three times, so the tree an arm
actually imported is witnessed four separate ways, and any disagreement ABORTS
the arm rather than reporting it.
    THE PATH.  os.path.realpath(mbirtorch.__file__) must sit inside the tree
    root the runner exported.
    THE BYTES.  hashlib sha256, first 12 hex, of tomography_model.py,
    _sharding.py and _memory_ledger.py read from beside that __file__.  The
    summary then checks these ACROSS arms: control and prefetch must differ in
    tomography_model.py and agree in _sharding.py (the patch touches only the
    driver), and streams must differ from both in both.
    THE SOURCE.  Tokens present in the live source of
    TomographyModel._sparse_forward_project_columns: 'batch_bounds' on the two
    overlays and not on control, 'gather_column_band_async' on streams alone.
    THE BEHAVIOUR.  Counts taken while the arm runs: how many times the async
    gather was entered (zero except on streams), how many copy streams the
    library created (zero except on streams), and the ledger constant
    COLUMN_GATHER_RESIDENTS -- 2 on control, 3 on both overlays -- read out of
    the arm's own _memory_ledger module.
Those four abort an arm that disagrees.  A fifth is recorded and printed but
never gating: the order in which one worker issued its first gathers and
projections, G P G P on control and G G P G P on both overlays, written by the
gather wrapper and the body wrapper independently.  It is kept out of the fatal
set because it is the only reading here that depends on how threads happen to be
scheduled, and the four above already settle which tree ran without touching
threading at all.

THE TRANSFER ROUTE, witnessed on every arm, because the whole question is
moot without it.  A stream buys nothing if the copies are not device-to-device
at all: the library probes the hardware once per configuration and falls back to
routing every transfer through host memory when a direct copy fails to
round-trip, and a host bounce synchronizes by construction.  So every arm prints
model.dev2dev_safe, records whether the library's own host-bounce warning fired
during the first forward, and reads the module flag that warning sets.  When the
route is the host bounce the harness prints an interpretation line saying that a
NO CHANGE result then says nothing about the copy-stream change -- and it
changes NO verdict.  The numbers decide; the line explains.
...
TERMS OF ART, each defined once, here.
    ...
    tree         one of the three source trees, selected by PYTHONPATH.
    configuration  a (geometry, device count, pixel batch) triple.  Each is run
                 on all three trees and gets one comparison block.
    composed     one whole timed reconstruction's wall.
    bracket      the forward funnel's per-device CUDA event span -- copies,
                 waits and kernels together.  The reading is the largest device's.
    busy         the sum of per-body-call event spans on one device -- the
                 projection kernels alone, without the copies that feed them.
                 The reading is the largest device's.
    ...
    repeat floor an arm's own pass-to-pass value distance.  Both forward kernels
                 ...
                 this is not zero and it is the floor every cross-tree distance
                 has to be read against.
    modeled peak the memory ledger's per-device peak for this arm's configuration.
    measured peak torch.cuda.max_memory_allocated on that device.

ENVIRONMENT KNOBS (all optional; the three tree roots are required).
    MG12_TREE_CONTROL / _PREFETCH / _STREAMS   the tree roots (required)
    MG12_ARMS=p4b04096-control,...   run only these arm tokens
    MG12_CONFIGS=p4b04096,c2b08192   run only these configurations
    MG12_ITERATIONS=3                VCD iterations per reconstruction
    MG12_WARM_REPEATS=2              timed reconstructions after the cold pass
    MG12_MAX_EVENT_PAIRS=400000      per-reconstruction event budget
    MG12_KEEP_ARTIFACTS=1            keep the sinograms and the value samples
    MG12_PIN=configure|env           how the device count is pinned; 'configure'
                                     is the default and the documented choice
                                     above.  'env' pins with MBIRTORCH_NUM_DEVICES
                                     instead, which leaves the model on the
                                     automatic branch so the library builds its
                                     own ledger.  Recorded on every row.
    MG12_SMOKE=1                     the local CPU smoke (tiny cell, few iters)
    MG12_DEVICE=cpu                  smoke device
"""
...
SMOKE = os.environ.get("MG12_SMOKE", "0") == "1"
SMOKE_CELL = (8, 24, 20)
DEVICE = os.environ.get("MG12_DEVICE", "cpu" if SMOKE else "cuda")

# The three trees.  The runner puts one of these on the child's PYTHONPATH and
# the child proves it imported from there; nothing else selects a tree.
TREES = ("control", "prefetch", "streams")
TREE_ENV = {"control": "MG12_TREE_CONTROL",
            "prefetch": "MG12_TREE_PREFETCH",
            "streams": "MG12_TREE_STREAMS"}
TREE_NOTE = {
    "control": "the committed tree: gather, project, gather, project",
    "prefetch": "prefetch_only.patch: the next gather is issued first, but on "
                "the same stream the projection runs on",
    "streams": "prefetch plus a dedicated copy stream per device and a "
               "per-batch event",
}
...
# The five configurations, in job order: the cell the stall was measured at
# first, then the rest of parallel, then cone.  Each runs on all three trees.
CONFIGS = (
    ...
    dict(token="p2b08192", geometry="parallel", n_dev=2, pixel_batch=8192),
    dict(token="c4b08192", geometry="cone", n_dev=4, pixel_batch=8192),
    dict(token="c2b08192", geometry="cone", n_dev=2, pixel_batch=8192),
)
SMOKE_CONFIGS = (
...
VCD_ITERATIONS = int(os.environ.get("MG12_ITERATIONS", "1" if SMOKE else "3"))
VCD_SEED = 13        # mg1's / mg5's / mg9's / mg10's / mg11's seed
WARM_REPEATS = max(2, int(os.environ.get("MG12_WARM_REPEATS", "2")))
...
PIN_MECHANISM = os.environ.get("MG12_PIN", "configure").strip().lower()
...
# effect this job is looking for is a fall from 16.3 to about 12.4 s, which is
# 24 percent -- an order of magnitude above this floor.
...
MG11_READING = {
    ...
    ("parallel", 4, 8192): dict(bracket_s=13.33, busy_s=10.83, composed_s=21.8,
                                wall_s=121),
    ("parallel", 2, 8192): dict(bracket_s=20.96, busy_s=18.31, composed_s=31.4,
                                wall_s=156),
    ("cone", 4, 8192): dict(bracket_s=16.47, busy_s=15.04, composed_s=39.0,
                            wall_s=192),
    ("cone", 2, 8192): dict(bracket_s=21.88, busy_s=19.43, composed_s=58.3,
                            wall_s=264),
}
GENERATOR_S = 70
...
RESULTS_DIR = os.environ.get(
    "MG12_RESULTS", os.path.dirname(os.path.abspath(__file__)))
...
# The library files whose bytes are fingerprinted per arm.  The first two are
# the ones the overlays change in the package; the third is the ledger the
# memory gate reads, and it changes too.
FINGERPRINT_FILES = ("tomography_model.py", "_sharding.py", "_memory_ledger.py")
...
MAX_EVENT_PAIRS = int(os.environ.get("MG12_MAX_EVENT_PAIRS", "400000"))
...
def build_arms():
    """Every arm, in job order.  CONFIGURATION-MAJOR: the three trees of one
...
def selected_arms():
    """The arms to run, narrowed by MG12_CONFIGS then by MG12_ARMS."""
    chosen = list(ARMS)
    raw = os.environ.get("MG12_CONFIGS", "").strip()
    if raw:
        ...
        for token in raw.split(","):
            ...
            if token not in known:
                raise ValueError(f"MG12_CONFIGS: {token!r} is not one of "
                                 f"{sorted(known)}")
            wanted.add(token)
        chosen = [a for a in chosen if a["config_token"] in wanted]
    raw = os.environ.get("MG12_ARMS", "").strip()
    if raw:
        ...
        for token in raw.split(","):
            ...
            if not token:
                continue
            if token not in known:
                raise ValueError(f"MG12_ARMS: {token!r} is not one of "
                                 f"{sorted(known)}")
            wanted.add(token)
...
def tree_root(tree):
    ...
    if not root:
        raise RuntimeError(
            ...
            f"import.  The sbatch builds the three trees and exports all three "
            f"roots; running this file by hand needs the same three exports.")
...
def _sino_path(geometry, cell):
    return os.path.join(RESULTS_DIR, f"_mg12_sino_{geometry}_{cell[0]}.npy")
...
def _sample_path(arm_id, index):
    return os.path.join(RESULTS_DIR, f"_mg12_sample_{arm_id}_p{index}.npy")
...
def source_tokens():
    """Which shape the live driver source carries, read out of the tree this
    process imported rather than out of any record of what was built.

    'batch_bounds' is the list the prefetch ordering builds and neither the
    control loop nor anything else in that method mentions; it is present on
    both overlays.  'gather_column_band_async' is the copy-stream gather and it
    is present on the streams tree alone.  A tree that passed the byte
    fingerprint but had the wrong driver would still be caught here, because
    this reads the compiled function's source rather than a file on disk."""
    import inspect

    from mbirtorch import _memory_ledger, _sharding
    from mbirtorch.tomography_model import TomographyModel

    source = inspect.getsource(TomographyModel._sparse_forward_project_columns)
    return dict(
        driver_has_batch_bounds=("batch_bounds" in source),
        driver_has_async_gather=("gather_column_band_async" in source),
        driver_has_plain_gather=("gather_column_band" in source),
...
# like a clean "no change".
TREE_SOURCE_EXPECTED = {
    "control": dict(driver_has_batch_bounds=False, driver_has_async_gather=False,
                    sharding_has_copy_stream=False, column_gather_residents=2),
    "prefetch": dict(driver_has_batch_bounds=True, driver_has_async_gather=False,
                     sharding_has_copy_stream=False, column_gather_residents=3),
    "streams": dict(driver_has_batch_bounds=True, driver_has_async_gather=True,
                    sharding_has_copy_stream=True, column_gather_residents=3),
}
...
class RegionInstrument:
    ...
    mg1_readout.py without change -- so mg12's forward bracket IS mg9's and
...
class BusyProbe:
    ...
    so the start event is reached only once they have finished.  On the streams
    tree the driver enqueues a wait on that same stream before this point, so
    the start event is again reached only once this batch's copies have landed.
    Either way the span that follows is the projection and not the transfer,
    which is what lets the busy column be compared across the three trees.
    ...
    def wrap(self, body, dev_index, device):
        ...
        wrapped._mg12_dev_index = dev_index
        wrapped._mg12_wrapped_body = body
        return wrapped
...
class GatherProbe:
    ...
    WHAT IS COUNTED, and why each count is here.
        ...
        gather_async_calls  entries into ``gather_column_band_async``, which
                            exists on the streams tree alone.  This is the
                            behavioural witness that the copy-stream path really
                            ran, and it is recorded by a different wrapper than
                            the byte fingerprint and the source tokens.
        wait_calls          entries into ``wait_for_column_band``: one per batch
                            on the streams tree, and the count must match the
                            gather count there.
    ...
    signal of its own -- on the streams tree the gather returns once its copies
    are issued rather than once they have landed, so that wall should fall.
...
def attach_forward_probes(model, torch_module, cuda, max_pairs):
    ...
    def effective_view_batch(body, num_pixels, band_cols, args):
        ...
        index = getattr(body, "_mg12_dev_index", None)
    ...
    def _outermost():
        ...
        streams tree's async wrapper calling the plain one underneath it.  Both
        are wrapped, and without this the streams tree would count and trace
        every batch twice."""
    ...
    # The streams tree's three entry points.  They do not exist on the other two
    # trees, so each is wrapped only when it is there -- and the fact that a
    # count stayed at zero is itself the witness that the tree was not the
    # streams one.
...
def copy_streams_created():
    ...
    cache.  Zero on the control and prefetch trees, which have no such cache at
    all, and one per CUDA device the gather touched on the streams tree."""
    from mbirtorch import _sharding
...
def _build_torch_model(geometry, cell, n_dev, cpu_devices=None):
    ...
    FLOORS that order it.  That is why the three trees' floors tables are
    irrelevant to every number in this job -- no arm consults one.  (In this
    build they are identical anyway: the overlays replace five files and
    ...
    MG12_PIN=env pins with MBIRTORCH_NUM_DEVICES instead, which leaves the model
...
def configure_arm(model, pixel_batch):
    ...
    all: the column gather is the DEFAULT on all three trees, so the shipped
    ...
    return dict(pixel_batch_attribute=int(pixel_batch),
                knobs=("model.configure_devices(num_devices=n) + "
                       ...
                       "column gather is the default on all three trees"))
...
def torch_worker(cfg):
    ...
    def reserved():
        ...
        streams tree introduces -- grows the reservation before it grows any
        single allocation, and a reader looking for that effect should not have
        to infer it."""
        if not cuda:
            return []
...
def _check_witnesses(result, record, tree, n_owners, num_slices, pixel_batch,
    ...
    Six checks, and each covers a different way the row could be wrong.
    """
    checks = {}
    ...
    # 4. THE COPY STREAMS.  Only the streams tree has them, and it must have one
    #    per CUDA device the gather touched, with one wait per gather and one
    #    open/close handshake per forward call.
    if tree == "streams":
        ...
        checks["no_async_gather"] = (record["gather_async_calls"] == 0)
        checks["no_copy_streams"] = (result.get("copy_streams_created", 0) == 0)

    # 5. THE PROBES ARE STILL THE ONES THE DRIVER CALLS.
    checks["probes_attached"] = all(record["probe_verify"].values())

    # THE ISSUE ORDER, recorded and reported but NOT part of the fatal set.
    # The control tree gathers and projects in turn, so a worker's trace opens
    # 'GP'; both overlays gather one batch ahead, so theirs opens 'GG'.  A pass
    # with a single batch has nothing to gather ahead, so there is nothing to
    # read then.  This is the only witness here that depends on thread
    # scheduling, and tree identity is already settled by three witnesses that
    # do not, so a disagreement is printed rather than fatal.
    traces = [t["sequence"] for t in (result.get("issue_trace") or ())
              if len(t.get("sequence") or "") >= 2]
    want_open = "GP" if tree == "control" else "GG"
...
def generator_worker(cfg):
    ...
    Every arm of that geometry -- on all three trees -- reconstructs THAT array,
    so no arm's timing or value carries an input difference.  One device is
    enough and is the point: a single-device forward never enters the sharded
    driver at all, so the three trees would produce the identical array here and
    the choice of the control tree records provenance rather than making a
    difference."""
    import numpy as np
...
def arm_env(cfg):
    ...
    default on all three trees, so its absence is the shipped configuration;
    setting it to '1' would measure the same thing but would also mean a rename
    in the library could silently stop mattering, and setting it to '0' would
    measure the banded walk.  Removal plus the child's assert is the only form
    with no ambiguity.

    MBIRTORCH_NUM_DEVICES is removed too, unless MG12_PIN=env asked for it: the
...
def run_one(cfg):
    os.makedirs(RESULTS_DIR, exist_ok=True)
    cfg_path = os.path.join(RESULTS_DIR, f"_cfg_mg12_{cfg['arm_id']}.json")
    out_path = os.path.join(RESULTS_DIR, f"_out_mg12_{cfg['arm_id']}.json")
...
def build_plan(arms):
    ...
    for arm in arms:
        ...
        if DEVICE != "cuda":
            ...
            # exercised without CUDA.  Off CUDA the copy streams do nothing by
            # construction, so the streams arm behaves as the prefetch arm and
...
def print_arm_table(row):
    ...
    print(f"      source: batch_bounds={tokens.get('driver_has_batch_bounds')} "
          f"async_gather={tokens.get('driver_has_async_gather')} "
...
def comparison(rows, config):
    """The three trees at ONE configuration, reduced to the numbers the two
    ...
    if control is None or not all(t in by_tree for t in TREES):
        entry["why"] = (f"this configuration needs all three trees and has "
    ...
    # THE STREAMS LINE.  The rule is stated in the module docstring and printed
    # beside the block: the fall has to exceed the two arms' spreads together,
    # each floored at SPREAD_FLOOR_FRAC of the control reading.
    s_bracket, s_bracket_sp = read("streams", "forward_bracket_max_s",
                                   "forward_bracket_spread_s")
    s_busy, _s_busy_sp = read("streams", "forward_busy_max_s",
                              "forward_busy_spread_s")
    margin = _floored(c_bracket_sp, c_bracket) + _floored(s_bracket_sp, c_bracket)
    entry["streams_margin_s"] = margin
    entry["streams_delta_s"] = ((c_bracket - s_bracket)
                                if c_bracket is not None and s_bracket is not None
                                else None)
    entry["streams_close_the_stall"] = bool(
        entry["streams_delta_s"] is not None
        and entry["streams_delta_s"] > margin)
    entry["streams_line"] = (
        f"STREAMS CLOSE THE STALL: {config['geometry']} n={config['n_dev']} "
        f"batch={config['pixel_batch']}: bracket {_fmt(c_bracket, '.2f')} -> "
        f"{_fmt(s_bracket, '.2f')} s (busy {_fmt(c_busy, '.2f')} -> "
        f"{_fmt(s_busy, '.2f')})"
        if entry["streams_close_the_stall"] else
        f"NO CHANGE: {config['geometry']} n={config['n_dev']} "
        f"batch={config['pixel_batch']}: bracket {_fmt(c_bracket, '.2f')} -> "
        f"{_fmt(s_bracket, '.2f')} s (busy {_fmt(c_busy, '.2f')} -> "
        f"{_fmt(s_busy, '.2f')})")

    # THE PREFETCH LINE.  The prediction was that nothing moves, so being INSIDE
    # the combined spread is what confirms it.
    p_bracket, p_bracket_sp = read("prefetch", "forward_bracket_max_s",
                                   "forward_bracket_spread_s")
    p_margin = _floored(c_bracket_sp, c_bracket) + _floored(p_bracket_sp,
                                                            c_bracket)
    entry["prefetch_margin_s"] = p_margin
    entry["prefetch_delta_s"] = ((p_bracket - c_bracket)
                                 if p_bracket is not None
                                 and c_bracket is not None else None)
    entry["prefetch_confirmed"] = bool(
        entry["prefetch_delta_s"] is not None
        and abs(entry["prefetch_delta_s"]) <= p_margin)
    entry["prefetch_line"] = (
        f"PREFETCH-ONLY PREDICTION: "
        f"{'CONFIRMED' if entry['prefetch_confirmed'] else 'REFUTED'} "
        f"({config['geometry']} n={config['n_dev']} "
        f"batch={config['pixel_batch']}: bracket {_fmt(c_bracket, '.2f')} -> "
        f"{_fmt(p_bracket, '.2f')} s, inside a combined spread of "
        f"{p_margin:.2f} s)"
        if entry["prefetch_confirmed"] else
        f"PREFETCH-ONLY PREDICTION: REFUTED "
        f"({config['geometry']} n={config['n_dev']} "
        f"batch={config['pixel_batch']}: bracket {_fmt(c_bracket, '.2f')} -> "
        f"{_fmt(p_bracket, '.2f')} s, outside a combined spread of "
        f"{p_margin:.2f} s)")

    # THE BUSY FLAG.  Neither overlay is supposed to change what the kernels
    # compute or how they are launched, so a busy change is a signal that
    # something other than the transfer schedule moved.
    busy_flags = []
    for tree in ("prefetch", "streams"):
        value, spread_value = read(tree, "forward_busy_max_s",
                                   "forward_busy_spread_s")
        if value is None or c_busy is None:
            continue
        allowed = _floored(c_busy_sp, c_busy) + _floored(spread_value, c_busy)
        if abs(value - c_busy) > allowed:
            busy_flags.append(
                f"BUSY CHANGED: {tree} busy {value:.2f} s against the control's "
                f"{c_busy:.2f} s, outside a combined spread of {allowed:.2f} s "
                f"-- the overlay altered compute, not just the transfer "
                f"schedule, so read every bracket in this block with that in "
                f"mind")
    entry["busy_flags"] = busy_flags

    # THE MEMORY GATE, which is the one gate here that can fail.
...
def route_note(entry):
    ...
    fact = (f"dev2dev_safe={safe}, host bounce in use={bounced} "
            f"(control, prefetch, streams)")
    if any(bounced) or not all(safe):
        reading = (
            "READ THIS BLOCK ACCORDINGLY: at least one arm routed its "
            "cross-device transfers through host memory.  A host bounce copies "
            "to the CPU and back and synchronizes by construction, so a copy "
            "stream has nothing to overlap and the expected streams result is "
            "NO CHANGE whatever the copy-stream code does.  A NO CHANGE below "
            "would therefore say nothing about the change itself.  This line "
            "explains; it changes no verdict.")
    else:
        reading = ("Direct device-to-device transfer is in use on all three "
                   "arms, which is the route the copy-stream change is about.")
    return fact, reading
...
def print_comparison(entry, values):
    ...
    for flag in entry.get("busy_flags") or ():
        print(f"      {flag}")
    if not entry.get("busy_flags"):
        print("      busy agrees across the three trees within the spreads, "
              "which is what an")
        print("      overlay that changes only the transfer schedule should do.")

    print("  MEMORY (modeled/measured; at or above 1.00 passes, below 1.00 is a "
          "FLOOR VIOLATION.")
    print("          Both overlays charge one more gathered cylinder "
          "(COLUMN_GATHER_RESIDENTS 2 -> 3);")
    print("          the streams arm is where a per-stream allocator pool or a "
          "deferred reuse could")
    print("          push the measured peak past that charge.")
    print("          The three byte columns are the readings ON the device that "
          "carried the ratio.)")
    ...
    print("          it.  The control arm's own pass-to-pass distance IS that "
          "floor: the same code")
    print("          reconstructing the same input twice, which is not zero "
          "because both forward")
    print("          kernels accumulate with float atomics.)")
    ...
    print(f"  {entry['streams_line']}")
    print(f"  {entry['prefetch_line']}")
...
def fingerprint_check(rows):
    ...
    is whether the three roots hold three different trees, and a job that
    pointed all three at one directory would report NO CHANGE three times and
    look tidy doing it.  So the digests are compared across arms against what
    the build is supposed to have produced:
        control vs prefetch   the driver differs; _sharding.py is IDENTICAL,
                              because prefetch_only.patch does not touch it
        control vs streams    both differ
        prefetch vs streams   both differ
    """
    live = [r for r in rows if not r.get("error")
            and r.get("arm_class") != "generator"]
    by_tree = {}
    ...
    compare("control", "prefetch", "tomography_model.py", False)
    compare("control", "prefetch", "_sharding.py", True)
    compare("control", "streams", "tomography_model.py", False)
    compare("control", "streams", "_sharding.py", False)
    compare("prefetch", "streams", "tomography_model.py", False)
    compare("prefetch", "streams", "_sharding.py", False)
...
def summarize(rows, out_path):
    ...
    print("\n" + "=" * 78)
    print(f"mg12 -- the copy-stream measurement, {len(live)} arms on "
          f"{RUN_LABEL} ({DEVICE})")
    print("=" * 78)
    print("  Three trees, measured in this job, against one another: the "
          "committed tree, the")
    print("  same tree with the gather issued one batch ahead, and the same "
          "tree again with the")
    print("  copies moved onto a stream of their own.  Nothing here is "
          "compared against a")
    print("  number measured anywhere else.")
    for tree in TREES:
        print(f"    {tree:<9} {TREE_NOTE[tree]}")
    if DEVICE != "cuda":
        print()
        print("  SMOKE RUN -- READ THE VERDICT LINES AS EXERCISE, NOT AS "
              "RESULT.  Off CUDA there")
        print("  are no streams: copy_stream returns None, the gather copies "
              "synchronously, and")
        print("  the streams tree behaves exactly as the prefetch tree.  There "
              "is also no device")
        print("  peak counter, so the memory ratio is NOT RESOLVABLE.  What "
              "the smoke does")
        print("  establish is the plumbing: every witness fires, the trees are "
              "distinguished, and")
        print("  the value distances are computed.")
    ...
    if check["ok"]:
        print("      the three trees differ from each other exactly where the "
              "build says they should")
        print("      (prefetch changes the driver and not _sharding.py; "
              "streams changes both)")
    else:
        for finding in check["findings"]:
            print(f"      PROVENANCE PROBLEM: {finding}")
        print("      A job whose three roots are not three different trees "
              "would report NO CHANGE")
        print("      three times and look tidy doing it.  Do not read the "
              "blocks below.")
    ...
    if off_order:
        ...
        print("  thread scheduling, and the byte fingerprint, the source "
              "tokens and the async gather")
        print("  count all agreed on which tree ran.  Read the per-arm traces "
              "above before")
...
def wall_estimate(generators, measured):
    ...
    A control arm is that base; the two overlay arms have never been run, so
    they take the same rule mg10 and mg11 used for an unmeasured arm: the low
    end assumes the arm costs what its control costs, the high end assumes half
    again as much.  On top of that sits an allowance for a COLD INDUCTOR CACHE:
    this job gets its own, so the first arm at each launch shape compiles from
    scratch where mg11 reused a warm one."""
...
def main():
    ...
    if "--dry-run" in sys.argv:
        ...
        print(f"mg12 plan: {len(measured)} measured arms + {len(generators)} "
        ...
        print(f"  device pin: {PIN_MECHANISM}; "
              ...
              f"because the column gather is the default on all three trees")
        print("  the three trees:")
        for tree in TREES:
            try:
                root = tree_root(tree)
        ...
        for cfg in measured:
            ...
            memo = ("" if reading is None else
                    f"  [mg11 read bracket {reading['bracket_s']:.2f} s vs busy "
                    f"{reading['busy_s']:.2f} s here]")
            print(f"  {cfg['arm_id']:<48} tree={cfg['tree']:<9} "
        ...
        print(f"  wall estimate {low / 60:.0f} to {high / 60:.0f} minutes "
              ...
              f"the overlay arms have never been run, so their high end "
              f"assumes half again as much; plus {COLD_CACHE_LOW_S / 60:.0f} "
              f"to {COLD_CACHE_HIGH_S / 60:.0f} minutes for a cold inductor "
              f"cache).")
        print("  if it must be cut: MG12_CONFIGS drops whole configurations by "
              "token and MG12_ARMS")
        print("  drops single arms.  Trim WHOLE CONFIGURATIONS -- a "
              "configuration missing any of")
        print("  its three trees has no comparison block at all, and the "
              "control arm is where the")
        print("  value floor comes from.  Trim in this order: c2b08192, "
              "p2b08192, c4b08192.")
        return
    stamp = time.strftime("%Y%m%d_%H%M%S")
    os.makedirs(RESULTS_DIR, exist_ok=True)
    out_path = os.path.join(RESULTS_DIR,
                            f"mg12_stream_gate_{RUN_LABEL}_{stamp}.jsonl")
    print(f"mg12 stream gate on {RUN_LABEL} ({DEVICE}); {len(measured)} arms "
          f"-> {out_path}", flush=True)
    rows = []
    # Rows write INCREMENTALLY: a truncated job still yields the configurations
    # it finished, which is why the arm order runs a configuration's three trees
    # back-to-back.
    with open(out_path, "w") as sink:
        ...
        sink.flush()
    if os.environ.get("MG12_KEEP_ARTIFACTS", "0") != "1":
