"""mg33 -- THE SORTED-SUBSET SPIKE ON THE SEGMENTED PARALLEL FORWARD.

WHY THIS RUN EXISTS.  mg32 measured the segmented contraction winning 1.68x
at the full pixel mask and losing (0.88x) at every subset point, because a
tile of subset pixels has no channel locality for any window (findings
§1.29).  Greg asked whether channel sorting recovers the subset case, and
the design note's answer is yes, in one specific form: sort per VIEW,
because a pixel's channel center depends on the view angle
(`pfwd_segmented_design.md` §2, the subset-path point).  This spike
measures that form.

THE DESIGN.  Outside the kernel, per call: argsort each view's channel
centers, gather the contract arrays (n_p, centers) into that order, and
hand the kernel the per-view permutation.  Inside the kernel: mg32's
window-contraction machinery unchanged, except the values tile is gathered
through the permutation INSIDE the view loop (each view has its own pixel
order), instead of loaded once before it.  The density arithmetic this
tests: a 1/64 subset's 12,051 pixels over 992 channels put a sorted
16-pixel tile in a 2-to-3-channel span, so the window covers it easily,
the adds fall far below the tap path's, and the contraction acts as a
segmented reduction (many pixels summing into few channels).

THE EXPECTED SHAPE OF THE RESULT, stated before the run: the subset
points are the domain and should win; the full-mask point should LOSE,
because a per-view gather re-reads the 3.1 GB values block per view (the
reload mg29 measured), and it is kept as the demonstration that the
sorted and chunked paths partition by call locality.  The reading that
matters is the COMBINED projection printed at the end: the best of the
sorted path (this run), the chunk path (mg32's winner, from its recorded
rows), and the shipped kernel, per ladder point -- the mixture a per-call
library selection would realize.

THE SORT'S OWN COST is timed separately per point (the argsort and the
two gathers) and reported beside the launches.  In production the
per-(subset, view) orderings compute once per reconstruction and are
reused every iteration, so the per-call charge amortizes; the raw
milliseconds are what this run records.

THE GATES, as mg32: values at 1e-5 against the shipped wrapper at every
point (the sort reorders the same commutative sums), and the shipped
baseline within 25 percent of mg20's 859.13 ms anchor.  The winner is
picked on the SUBSET mixture (the /4, /16, /64 points with their
production weights), because the full-mask point belongs to the other
path by design.

Run:
    <torch python> mg33_pfwd_sorted.py           on one GPU
    MG33_DRY=1 <python> mg33_pfwd_sorted.py      print the plan and stop
    MG33_SMOKE=1 <python> mg33_pfwd_sorted.py    tiny CPU plumbing pass

Configuration is by environment variable only; there is no command line.
    MG33_RESULTS=<dir>      where the jsonl and the ncu log go
    MG33_SMOKE=1 / MG33_DRY=1
    MG33_REPEATS=3          timed repeats per (configuration, ladder point)
    MG33_CONFIGS=a,b        a subset of the configurations, by name
    MG33_NCU=1              the counter attempt on the winner (default on)
"""
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
SMOKE = os.environ.get("MG33_SMOKE", "0") == "1"
DRY = os.environ.get("MG33_DRY", "0") == "1"
...
WARMUP_REPEATS = 1
TIMED_REPEATS = max(1, int(os.environ.get("MG33_REPEATS", "3")))
...
#: num_warps, num_stages, ieee) -- every configuration runs the sorted
#: path.  Sorted subset tiles span 2 to 3 channels, so windows of 16 and
#: 32 both cover them and the sweep asks which the hardware prefers;
#: BLOCK_P grows to 32 under the small windows because the span no longer
#: scales with the pixel count.  The chunk is grid granularity only here
#: (the values tile is gathered per view either way).
SWEEP_CONFIGS = (
    ("srt_p16w16r64c16", 16, 16, 16, 64, 4, 1, True),
    ("srt_p16w32r64c16", 16, 32, 16, 64, 4, 1, True),
    ("srt_p32w16r64c16", 16, 16, 32, 64, 4, 1, True),
    ("srt_p32w32r64c16", 16, 32, 32, 64, 4, 1, True),
    ("srt_p16w16r128c16", 16, 16, 16, 128, 8, 1, True),
    ("srt_p16w32r128c16", 16, 32, 16, 128, 8, 1, True),
    ("srt_p32w16r128c16", 16, 16, 32, 128, 8, 1, True),
    ("srt_p32w32r128c16", 16, 32, 32, 128, 8, 1, True),
)

# ── the counter attempt on the winner (mg28's machinery, one variant) ─────────
NCU_ENABLED = os.environ.get("MG33_NCU", "1") == "1"
NCU_LAUNCHES = 5
NCU_TIMEOUT_S = 420
...
def viewloop_kernel():
    ...
    @_jit
    def _pfwd_sorted_kernel(n_p_ptr, centers_ptr, w_p_c_ptr,
                               weight_scale_ptr, values_ptr, perm_ptr,
                               out_ptr, fallback_count_ptr,
        ...
        for dv in range(VIEW_CHUNK):
            ...
            # The contract arrays arrive PRE-SORTED per view, so their
            # loads stay contiguous; the permutation maps each sorted
            # position back to its values row.  The values tile is
            # therefore gathered INSIDE the view loop -- each view has its
            # own pixel order -- which is what confines this kernel to
            # calls whose values block is cache-resident (the design
            # note's subset-path point).
            ...
            vals = tl.load(values_ptr
                           + row_idx.to(tl.int64)[:, None] * num_cols
    ...
    _KERNEL_HOLDER["kernel"] = _pfwd_sorted_kernel
    return _pfwd_sorted_kernel
...
def viewloop_launch(values, pixel_indices, view_params_batch, args,
    ...
    # THE SORT, per view: argsort each view's channel centers, gather the
    # contract into that order, and keep the permutation for the kernel's
    # values-row gather.  Timed separately so the launch readings are the
    # kernel's; the caller records sort_ms beside them.  In production
    # these orderings compute once per reconstruction and are reused
    # every iteration.
    if torch.cuda.is_available():
        torch.cuda.synchronize(values.device)
    sort_start = time.perf_counter()
    order = torch.argsort(n_p, dim=1)
    n_p = torch.gather(n_p, 1, order)
    centers = torch.gather(centers, 1, order)
    perm = order.to(torch.int32).contiguous()
    if torch.cuda.is_available():
        torch.cuda.synchronize(values.device)
    sort_ms = (time.perf_counter() - sort_start) * 1e3
    viewloop_launch.last_sort_ms = sort_ms
    ...
    launch_key = ("mg33_sorted", values.device.index,
    ...
    with torch.cuda.device(values.device), guard:
        kernel[grid](
            *contract, values, perm, out, fb_counter,
...
def run_sweep(sink):
    ...
    configs = _strict_subset("MG33_CONFIGS",
    ...
    for divisor, mixture_weight in sorted(PIXEL_LADDER):
        ...
        for name in configs:
            ...
            point.update(divisor=divisor, mixture_weight=mixture_weight,
                         ...
                         sort_ms=getattr(viewloop_launch, "last_sort_ms",
                                         None),
    ...
    subset_weight_sum = sum(w for d, w in PIXEL_LADDER if d != 1)
    base_subset = sum(p["median_ms"] * p["mixture_weight"]
                      for p in baseline_points
                      if p["divisor"] != 1) / subset_weight_sum
    for name in configs:
        row = config_rows[name]
        if row["points"]:
            ...
            subset_points = [p for p in row["points"] if p["divisor"] != 1]
            if subset_points:
                row["subset_mixture_ms"] = sum(
                    p["median_ms"] * p["mixture_weight"]
                    for p in subset_points) / subset_weight_sum
                row["subset_mixture_speedup"] = (base_subset
                                                 / row["subset_mixture_ms"])
...
def ncu_on_winner(winner, results_dir):
    ...
    if not NCU_ENABLED:
        leg.update(attempted=False, reason="MG33_NCU=0")
        return leg
    ...
    pattern = "regex:" + re.escape("_pfwd_sorted_kernel")
    ...
    log_path = os.path.join(results_dir, f"mg33_ncu_{winner}.log")
...
def pick_winner(config_rows):
    ...
    for name, row in config_rows.items():
        ...
        if row.get("gate_exempt"):
            continue
        mix = row.get("subset_mixture_speedup",
                      row.get("mixture_speedup"))
...
def summarize(header, baseline_points, config_rows, leg, out_path):
    print(f"\n===== mg33 sorted-subset spike ({out_path}) =====")
    ...
    if winner:
        print(f"\n  the winning configuration by SUBSET mixture: {winner} "
              f"({winner_mix:.2f}x on the /4, /16, /64 points)")
        # The combined projection: the best of the shipped kernel, mg32's
        # chunk-path winner, and this run's sorted winner, per ladder
        # point -- the mixture a per-call library selection would realize.
        # The mg32 and baseline constants are that run's recorded medians
        # (rows/mg32_pfwd_segmented_h002_20260818_172015.jsonl, config
        # s_p16w32r128c32 and its baseline points).
        mg32_chunk = {1: 515.66, 4: 244.83, 16: 62.93, 64: 17.07}
        mg32_base = {1: 864.70, 4: 216.44, 16: 55.12, 64: 14.78}
        sorted_points = {p["divisor"]: p["median_ms"]
                         for p in config_rows[winner]["points"]}
        line2 = (f'{"divisor":>9}{"shipped ms":>12}{"chunk ms":>10}'
                 f'{"sorted ms":>11}{"selected":>10}{"vs shipped":>12}')
        print("\n===== the combined projection: best path per point =====")
        print(line2)
        print("-" * len(line2))
        total_base, total_sel, weights = 0.0, 0.0, 0
        for divisor, weight in sorted(PIXEL_LADDER):
            shipped = mg32_base.get(divisor)
            chunk = mg32_chunk.get(divisor)
            srt = sorted_points.get(divisor)
            candidates = {"shipped": shipped, "chunk": chunk,
                          "sorted": srt}
            known = {k: v for k, v in candidates.items() if v is not None}
            best_name = min(known, key=known.get)
            total_base += shipped * weight
            total_sel += known[best_name] * weight
            weights += weight
            print(f'{divisor:>9}{_fmt(shipped, 12, "f", 2)}'
                  f'{_fmt(chunk, 10, "f", 2)}{_fmt(srt, 11, "f", 2)}'
                  f'{best_name:>10}'
                  f'{_fmt(shipped / known[best_name], 12, "f", 2)}')
        print("-" * len(line2))
        print(f"  selected-mixture speedup over the shipped kernel: "
              f"{(total_base / total_sel):.2f}x  (mg32 constants are that "
              "run's recorded medians; this run's own baseline is the "
              "anchor check)")
...
def _dry_run():
    configs = _strict_subset("MG33_CONFIGS", [c[0] for c in SWEEP_CONFIGS])
    print(f"mg33 sorted-subset spike: device {DEVICE}, cell {tuple(cell())}, "
    ...
    print(f"  counter attempt on the winner: "
          f"{'on' if NCU_ENABLED else 'off (MG33_NCU=0)'}")
...
def main():
    ...
    if not SMOKE:
        import torch
        if not torch.cuda.is_available():
            print("this run needs CUDA; use MG33_SMOKE=1 for the CPU "
                  "plumbing pass")
            return 2
    os.makedirs(RESULTS_DIR, exist_ok=True)
    stamp = time.strftime("%Y%m%d_%H%M%S")
    out_path = os.path.join(RESULTS_DIR,
                            f"mg33_pfwd_sorted_{RUN_LABEL}_{stamp}.jsonl")
    print(f"mg33 sorted-subset spike on {RUN_LABEL} ({DEVICE}) -> {out_path}",
          flush=True)
