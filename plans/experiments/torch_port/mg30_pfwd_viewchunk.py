"""mg30 -- THE VIEW-CHUNK VARIANT OF THE PARALLEL FORWARD KERNEL: THE
SECOND SPIKE.

WHY THIS RUN EXISTS.  mg29 measured the two ends of a trade and found both
lose.  The SHIPPED kernel gives every program one view: the output planes
stay cache-resident (0.5 GB of DRAM writes) but the values block is
re-read once per view (377 GB of DRAM reads; findings §1.26).  mg29's
view-LOOP variant gave every program all 128 views: the values re-read
collapsed to 38 GB, but the concurrent programs then walked 128 different
output planes at once, the planes stopped fitting the 50 MB L2, and the
atomic adds thrashed to DRAM instead -- 262 GB of writes, for a net win of
only 1.10x.  The traffic moved from the read side to the write side almost
one for one.

The remedy is the middle the two ends bracket: a view CHUNK.  Each program
loads its values tile once and loops over a chunk of views, with the chunk
index a grid axis.  A chunk of c views amortizes the tile load c times
while the concurrently active output planes stay few enough to live in L2.
The shipped kernel is this design at c=1 and mg29's variant is c=128; the
sweep walks the interior.  128 divides by every swept chunk, so the loop
needs no view guard.

Greg's B7 ruling covers this step: it is the same increment's spike,
iterated on what the first sweep measured, and it still touches no library
file.  The library change happens only if a configuration gates, and then
re-gates composed (lessons §5).

THE SWEEP.  Chunks 4, 8, 16, and 32, each at the two best mg29 tiles
((16, 64) at 4 warps and (8, 128) at 8 warps).  Eight configurations, each
gated on values and timed on the ladder exactly as mg29's were.
...
The confirmation is now two-sided: DRAM read near 128-over-chunk times
the values block (the amortization), AND DRAM write back near the
shipped kernel's half-gigabyte class rather than mg29's 262 GB (the
output planes resident again).
...
Run:
    <torch python> mg30_pfwd_viewchunk.py           on one GPU
    MG30_DRY=1 <python> mg30_pfwd_viewchunk.py      print the plan and stop
    MG30_SMOKE=1 <python> mg30_pfwd_viewchunk.py    tiny CPU plumbing pass
...
Configuration is by environment variable only; there is no command line.
    MG30_RESULTS=<dir>      where the jsonl and the ncu log go
    MG30_SMOKE=1 / MG30_DRY=1
    MG30_REPEATS=3          timed repeats per (configuration, ladder point)
    MG30_CONFIGS=a,b        a subset of the configurations, by name
    MG30_NCU=1              the counter attempt on the winner (default on)
"""
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
SMOKE = os.environ.get("MG30_SMOKE", "0") == "1"
DRY = os.environ.get("MG30_DRY", "0") == "1"
...
WARMUP_REPEATS = 1
TIMED_REPEATS = max(1, int(os.environ.get("MG30_REPEATS", "3")))
...
#: The swept configurations: (name, VIEW_CHUNK, BLOCK_P, BLOCK_R,
#: num_warps, num_stages).  Chunks 4 to 32 walk the interior between the
#: shipped kernel (chunk 1) and mg29's whole-batch loop (chunk 128), at the
#: two tile shapes mg29 ranked best.  Every chunk divides the 128-view
#: batch, so the view loop needs no guard.
SWEEP_CONFIGS = (
    ("c4p16r64w4", 4, 16, 64, 4, 1),
    ("c8p16r64w4", 8, 16, 64, 4, 1),
    ("c16p16r64w4", 16, 16, 64, 4, 1),
    ("c32p16r64w4", 32, 16, 64, 4, 1),
    ("c4p8r128w8", 4, 8, 128, 8, 1),
    ("c8p8r128w8", 8, 8, 128, 8, 1),
    ("c16p8r128w8", 16, 8, 128, 8, 1),
    ("c32p8r128w8", 32, 8, 128, 8, 1),
)

# ── the counter attempt on the winner (mg28's machinery, one variant) ─────────
NCU_ENABLED = os.environ.get("MG30_NCU", "1") == "1"
NCU_LAUNCHES = 5
NCU_TIMEOUT_S = 420
...
def viewloop_kernel():
    ...
    @_jit
    def _pfwd_viewchunk_kernel(n_p_ptr, centers_ptr, w_p_c_ptr,
        ...
        # The voxel cylinders, read ONCE for the whole chunk: under parallel
        # beam the values do not depend on the view.  The chunk is what
        # keeps the concurrently active output planes few enough for the
        # cache while the tile load still amortizes.
    ...
    _KERNEL_HOLDER["kernel"] = _pfwd_viewchunk_kernel
    return _pfwd_viewchunk_kernel
...
def viewloop_launch(values, pixel_indices, view_params_batch, args, config):
    ...
    _name, view_chunk, block_p, block_r, num_warps, num_stages = config
    ...
    launch_key = ("mg30_viewchunk", values.device.index,
                  int(args["psf_radius"]), view_chunk, block_p, block_r,
                  num_warps, num_stages, int(num_views), int(num_pixels),
...
def run_sweep(sink):
    ...
    configs = _strict_subset("MG30_CONFIGS",
...
def ncu_on_winner(winner, results_dir):
    ...
    if not NCU_ENABLED:
        leg.update(attempted=False, reason="MG30_NCU=0")
        return leg
    ...
    pattern = "regex:" + re.escape("_pfwd_viewchunk_kernel")
    ...
    log_path = os.path.join(results_dir, f"mg30_ncu_{winner}.log")
...
def summarize(header, baseline_points, config_rows, leg, out_path):
    ...
    line = (f'{"config":<{NAME_COL}}{"chunk":>7}{"P":>5}{"R":>5}{"warps":>7}'
    ...
    for name, row in config_rows.items():
        ...
        if row.get("error"):
            ...
            continue
        chunk, p, r, warps, _stages = row["params"]
        check = "ok" if row["values_ok"] else "VALUES GATE FAILED"
        print(f'{name:<{NAME_COL}}{chunk:>7}{p:>5}{r:>5}{warps:>7}'
...
def _dry_run():
    configs = _strict_subset("MG30_CONFIGS", [c[0] for c in SWEEP_CONFIGS])
    print(f"mg30 view-loop spike: device {DEVICE}, cell {tuple(cell())}, "
    ...
    print(f'  {"config":<{NAME_COL}}{"chunk":>7}{"BLOCK_P":>9}'
          f'{"BLOCK_R":>9}{"warps":>7}{"stages":>8}')
    for name, c, p, r, w, s in SWEEP_CONFIGS:
        marker = "" if name in configs else "  (not selected)"
        print(f"  {name:<{NAME_COL}}{c:>7}{p:>9}{r:>9}{w:>7}{s:>8}"
              f"{marker}")
    ...
    print(f"  counter attempt on the winner: "
          f"{'on' if NCU_ENABLED else 'off (MG30_NCU=0)'}")
...
def main():
    ...
    if not SMOKE:
        import torch
        if not torch.cuda.is_available():
            print("this run needs CUDA; use MG30_SMOKE=1 for the CPU "
                  "plumbing pass")
            return 2
    os.makedirs(RESULTS_DIR, exist_ok=True)
    stamp = time.strftime("%Y%m%d_%H%M%S")
    out_path = os.path.join(RESULTS_DIR,
                            f"mg30_pfwd_viewchunk_{RUN_LABEL}_{stamp}.jsonl")
    print(f"mg30 view-loop spike on {RUN_LABEL} ({DEVICE}) -> {out_path}",
          flush=True)
